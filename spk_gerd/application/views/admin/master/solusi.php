              ...
              <tbody>
                ...
                foreach($solusi as $g):
                  ...
                  <tr>
                    ...
                    <td><?=$g->solusi?></td>
                    <td>
                      <button type="button" class="btn btn-info btn-icon mb-2 me-4" onclick="edit(`<?=$g->kode_solusi?>`)" data-bs-toggle="modal" data-bs-target="#rotateleftModal">
                      ...
                      <button class="btn btn-danger btn-icon mb-2 me-4" onclick="hapus(`<?=$g->kode_solusi?>`)">
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Form Solusi Pengobatan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
          <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
      </div>
      <form action="javascript:void(0)" method="POST" id="form_solusi">
...
<script>
    ...
    function tambah(){
      $("#form_solusi")[0].reset();
      $(".modal-title").text('TAMBAH SOLUSI PENGOBATAN');
      metode = "tambah";
      $("#kode_solusi").removeAttr('readonly');
    }

    function edit(kode_solusi){
      $("#form_solusi")[0].reset();
      $(".modal-title").text('EDIT SOLUSI PENGOBATAN');
      metode = "edit";
      $.ajax({
        url:"<?=base_url('admin/Master/edit_solusi')?>/"+kode_solusi,
        type:"GET",
        dataType:"JSON",
        success: function(data){
          $("#kode_solusi").attr('readonly','readonly');
          $("#kode_solusi").val(`${data.kode_solusi}`);
    ...
    $("#form_solusi").submit(function(e){
     e.preventDefault();
     $.ajax({
      url:"<?=base_url('admin/Master/save_solusi/')?>"+metode,
      type:"POST",
      data:$("#form_solusi").serialize(),
      dataType:"JSON",
      success:function(data){
        if(data.status == true){
         Swal.fire(
    ...
    function hapus(kode_solusi){
      Swal.fire({
        ...
        if (result.isConfirmed) {
         $.ajax({
          url:"<?=base_url('admin/Master/delete_solusi');?>/"
          +kode_solusi,
          type:"POST",
          dataType:"JSON",
          cache:false,
